  ...
  factory: (require) => {
    ...
    function patchSendSession(conversation, sessions) {
      ...
      proto.sendSession = async function (session, text, imageIds, mode) {
        ...
        clearFiles()
        // DSH sendSession contract: settleSubmit reads outcome.kind to decide
        // commitSend (clears the composer). Resolving with undefined left the
        // input box stale (#6) and crashed it entirely (#7).
        return { kind: 'success' }
      }
    }
