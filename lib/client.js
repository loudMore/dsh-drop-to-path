 ...
 *    AND shown as removable chips above the composer (no preview needed).
 *
 ...
 * native send path.
 *
 * DSH's full-screen drop overlay is closed by re-dispatching an empty drop
 * event after interception, so the page never gets stuck behind the overlay.
 */
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ---- file chips (visual queue above the composer) ----

    function renderChips() {
      var ta = findComposer()
      if (!ta || !ta.parentElement) return
      var old = ta.parentElement.querySelector('[' + CHIPS_ATTR + ']')
      if (old) old.remove()
      if (fileQueue.length === 0) return
      var bar = document.createElement('div')
      bar.setAttribute(CHIPS_ATTR, '1')
      bar.style.cssText = 'display:flex;gap:6px;flex-wrap:wrap;padding:4px 12px 0;max-width:100%'
      fileQueue.forEach(function (item) {
        var chip = document.createElement('span')
        chip.style.cssText = 'display:inline-flex;align-items:center;gap:6px;max-width:240px;' +
          'background:rgba(92,108,213,.12);border:1px solid rgba(92,108,213,.35);' +
          'color:var(--dsw-alias-fg-primary,#26231f);border-radius:999px;padding:3px 8px;' +
        ...
        remove.textContent = '✕'
        remove.title = '移除'
        remove.style.cssText = 'border:0;background:transparent;cursor:pointer;color:inherit;font:inherit;padding:0 2px'
    ...
    // Rebuild chips if a React re-render of the composer removed them.
    var chipsTimer = null
    function ensureChipsObserver(ta) {
      var node = ta.parentElement
      var observer = new MutationObserver(function () {
        if (fileQueue.length > 0 && node.querySelector('[' + CHIPS_ATTR + ']') === null) {
    ...
    function installFileInterception() {
      var onDrop = function (event) {
        ...
        // Upload non-image files strictly in order, inserting each path
        // before the next upload starts, so the composer lines stay ordered.
            ...
            setTimeout(function () {
              try {
                ...
                images.forEach(function (f) {
                  chain = chain.then(function () { return upload(f) }).then(function (path) {
                    insertPath(path)
                    addFile(path, f.name)
                  }).catch(function (err) {
                    console.error('[drop-to-path] image upload failed:', err)
                    showNotice('[dsh-drop-to-path] 图片上传失败: ' + (err && err.message ? err.message : String(err)))
                  })
                })
              }
            }, 0)
        ...
        // DSH shows its full-screen drop overlay while a drag is in flight and
        // hides it in its own drop/dragend handlers. We stopped propagation of
        // the real drop (so DSH never adds the non-image file), which leaves
        // the overlay stuck. DSH resets the overlay unconditionally on
        // `dragend` (window listener) — dispatch one so the overlay closes
        // and the page never gets stuck. No file payload is involved.
        setTimeout(function () {
          try {
